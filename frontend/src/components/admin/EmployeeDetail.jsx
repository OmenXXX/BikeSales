    ...
    if (!isEditing) {
        return (
            <Tooltip text={tooltip || `View ${placeholder}`} fullWidth>
                <div className={containerClasses + ' cursor-help'}>
                    {content}
                </div>
            </Tooltip>
        );
...
const EmployeeDetail = ({
    ...
    const handleSave = async () => {
        ...
        if (isSelfUpdate && emailChanged) {
            const confirmed = await showConfirm({
                ...
                message: 'You are updating your own email address. For security reasons, you will be logged out and asked to sign in again with your new credentials. Proceed?'
            });
            if (!confirmed) return;
        }
    ...
    const handleToggleStatus = async () => {
        ...
        const confirmed = await showConfirm({
            title: isCurrentlyActive ? 'Suspend Access' : 'Activate Access',
        ...
        <div className="max-w-7xl mx-auto space-y-4 animate-pulse pb-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 space-y-6">
                    <div className="w-32 h-32 rounded-full bg-slate-100 mx-auto"></div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 space-y-6">
                    ...
                    <div className="space-y-4">
                        {[...Array(4)].map((_, i) => (
                            <div key={i} className="h-16 bg-slate-50 rounded-2xl flex items-center px-4 gap-4">
                            ...
                            <div className="grid grid-cols-3 gap-6 mb-8 items-start">
                                ...
                                <div className="col-span-1 flex flex-col justify-start space-y-3">
                                    <div className="space-y-1">
                                        <Tooltip text={isEditing ? "Edit Employee Initials (max 3 chars)" : "Employee Initials"} fullWidth>
                                            <div className="w-full h-12 lg:h-10 px-4 rounded-xl text-xs font-black bg-slate-50 border border-slate-50 transition-all flex items-center justify-between cursor-help">
                                                {isEditing ? (
                                                    <input
                                                        type="text"
                        ...
                        <ChangePasswordModal
                            ...
                            onSubmit={async (pw) => {
                                if (isSelf) {
                                    const confirmed = await showConfirm({
                                        title: 'Security Alert',
                                        message: 'You are updating your own password. For security reasons, you will be logged out. Proceed?'
                                    });
                                    if (!confirmed) return;
                                }
