...
import AdminProductPreferences from '../components/admin/AdminProductPreferences';
...
const Admin = () => {
    ...
    // Sync activeModule with activeView for data fetching and breadcrumbs
    useEffect(() => {
        if (activeView === 'employees' || activeView === 'detail') {
            setActiveModule('employees');
        }
    }, [activeView]);

    useEffect(() => {
        if (activeModule === 'employees' && allEmployees.length === 0) {
            fetchEmployees();
        }
    }, [activeModule]);
    ...
    const filteredEmployees = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter(emp => {
                const f = emp.fieldData;
                const firstName = (f.Name_First || '').toLowerCase();
                const middleName = (f.Name_Middle || '').toLowerCase();
                const lastName = (f.Name_Last || '').toLowerCase();
                const fullName = `${firstName} ${middleName} ${lastName}`.replace(/\s+/g, ' ').trim();
                const email = (f.EmailAddress || '').toLowerCase();
                const phone = String(f.PhoneNumber || '');
    ...
    const handleEmployeeClick = async (employee) => {
        ...
        try {
            // Fetch fresh record from FileMaker for detail view
            const response = await getRecords('Employees', {
                query: [{ PrimaryKey: `==${employee.fieldData.PrimaryKey}` }]
            });
    ...
    const handleEmployeeUpdate = (updatedRecord) => {
        // Update the main list in memory
        setAllEmployees(prev => prev.map(emp =>
            emp.recordId === updatedRecord.recordId ? updatedRecord : emp
        ));

        // Update the currently selected employee in memory
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                <header className="h-24 bg-white/40 backdrop-blur-sm px-6 md:px-10 flex items-center justify-between border-b border-slate-100 relative z-40">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight capitalize">
                            {activeView === 'detail' ? 'Employee Profile' :
                                (activeView === 'structure' ? 'System Structure' : 
                                 activeView === 'product-preferences' ? 'Product Preferences' : 
                                 `${activeView.replace('-', ' ')} Operations`)}
                        </h2>
                    </div>
                ...
                <main className="flex-1 overflow-hidden relative">
                    ...
                    <div className="absolute inset-0 overflow-y-auto scroll-smooth">
                        ...
                        {activeView === 'product-preferences' && <AdminProductPreferences primaryColor={primaryColor} />}
                                ...
                                <div className="hidden lg:block w-80 h-full border-r border-slate-100 bg-white z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
                                    <AdminEmployeeListSidebar
                                        ...
                                        onSelectEmployee={async (emp) => {
                                            ...
                                            try {
                                                const response = await getRecords('Employees', {
                                                    query: [{ PrimaryKey: `==${emp.fieldData.PrimaryKey}` }]
                                                });
                                                if (response.success && response.data.length > 0) {
                                                    setSelectedEmployee(response.data[0]);
                                                    setIsEditing(false);
